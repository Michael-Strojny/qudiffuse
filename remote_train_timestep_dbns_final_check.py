...
# QuDiffuse imports
from qudiffuse.models import MultiResolutionBinaryAutoEncoder, BinaryLatentManager, HierarchicalDBN
from qudiffuse.models.timestep_specific_dbn_manager import TimestepSpecificDBNManager
from qudiffuse.utils.common_utils import cleanup_gpu_memory
...
class TimestepDataGenerator:
    ...
    def generate_timestep_data(self, timestep: int, clean_latents: List[List[torch.Tensor]]) -> List[Tuple[List[torch.Tensor], List[torch.Tensor]]]:
        ...
        for clean_codes in tqdm(clean_latents, desc=f"Timestep {timestep}", leave=False):
            ...
            # For timestep t=1, target is original (clean) codes
            if timestep == 1:
...
class CompleteDBNPipeline:
    ...
    def __init__(self, autoencoder_checkpoint: str, config: Optional[DBNTrainingConfig] = None):
        ...
        # Initialize the BinaryLatentAutoencoder
        # Assuming default base_channels=64, latent_dim=256 from train_v100_gpu_fixed.py
        self.autoencoder = BinaryLatentAutoencoder(base_channels=64, latent_dim=256)
        self.autoencoder.load_state_dict(checkpoint['model_state_dict'])
        self.autoencoder.to(self.device)
        self.autoencoder.eval()
        
        # Manually define latent shapes for the DBN.
        # The BinaryLatentAutoencoder outputs a flattened latent of size latent_dim.
        # We assume the DBN operates on this single flattened latent space.
        # The shape should be (channels, height, width) for the DBN.
        # Since it's a flattened latent_dim, let's represent it as (latent_dim, 1, 1).
        # If the DBN expects multiple levels, this will need further adjustment.
        self.latent_shapes = [(256, 1, 1)] # (latent_dim, 1, 1) based on default latent_dim=256
        
        logger.info("✅ Complete DBN pipeline initialized")
        logger.info(f"   Autoencoder latent shapes (for DBN): {self.latent_shapes}")
...
class BinaryLatentAutoencoder(nn.Module):
    """
    Authentic Binary Latent Autoencoder for QuDiffuse
    Zero mocks, full implementation with hierarchical binary latent space
    """
    def __init__(self, base_channels: int = 64, latent_dim: int = 256):
        super().__init__()
